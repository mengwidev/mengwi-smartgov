                    ...
                    ->schema([
                        Forms\Components\Section::make('')
                            ->schema([
                                TextInput::make('name')
                                    ->label('Nama')
                                    ->helperText('Isikan nama tanpa gelar depan dan gelar belakang.')
                                    ->required(),

                                Forms\Components\Grid::make(12)
                                    ->schema([
                                        Select::make('last_education_id')
                                            ->relationship('lastEducation', 'name', fn($query) => $query->orderBy('id'))
                                            ->searchable()
                                            ->preload()
                                ...
                                Forms\Components\Grid::make(12)
                                    ->schema([
                                        Select::make('employment_position_id')
                                            ->relationship('employmentPosition', 'position_name', fn($query) => $query->orderBy('id'))
                                            ->searchable()
                                            ->preload()
                                            ->label('Jabatan')
                                            ->required()
                                        ...
                                        Select::make('banjar_id')
                                            ->relationship('banjar', 'banjar_name', fn($query) => $query->orderBy('id'))
                                            ->options(function () {
                                                return DB::table('ref_banjar')
                                                    ->select('id', DB::raw('CONCAT("Br. ", banjar_name) as banjar_name'))
                        ...
                        Forms\Components\Section::make('')
                            ->schema([
                                TextInput::make('att_pin')
                                    ->label('PIN')
                                    ->required()
                                    ->helperText('PIN dapat diperoleh dari admin mesin absen.'),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('employmentPosition.position_name')  // Displays position_name
                    ->label('Jabatan')
                    ->searchable(),
                ...
                Tables\Columns\TextColumn::make('banjar.banjar_name')  // Displays banjar_name
                    ->label('Alamat')
                    ->searchable()
                    ->getStateUsing(fn($record) => 'Br. ' . $record->banjar->banjar_name),
            ])
            ->filters([
                Tables\Filters\SelectFilter::make('banjar_id')
                    ->label('Banjar')
                    ->options(fn() => DB::table('ref_banjar')->pluck('banjar_name', 'id'))
                    ->searchable(),

                // Filter for employment_position_id
                Tables\Filters\SelectFilter::make('employment_position_id')
                    ->label('Jabatan')
                    ->options(fn() => DB::table('ref_employment_position')->pluck('position_name', 'id'))
                    ->searchable(),

                // Filter for last_education_id
                Tables\Filters\SelectFilter::make('last_education_id')
                    ->label('Pendidikan Terakhir')
                    ->options(fn() => DB::table('ref_last_education')->pluck('name', 'id'))
                    ->searchable(),
            ])
            ->actions([
